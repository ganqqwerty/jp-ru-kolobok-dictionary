...
WRUN-39 — Use `scripts/wadoku_extension_pilot.py` with the old and new scope files to select 100 new entries across internal slots, suffixes, prefixes, multiple senses, linked examples and cross-references. Pass its JSON to `wadoku_translate_window.py --scope-id NEW_ID --candidate-subset-file PILOT_SELECTION --articles-per-batch 6 --context-budget 192000 --max-batches 100`. Wait for complete pilot coverage, then inspect every translation in the main thread and check usage and rejection rates. The pilot uses the same full-scope classification and source contexts.

WRUN-40 — Prepare the full new run with `scripts/wadoku_translate_window.py --scope-id NEW_ID --candidate-scope --prepare-only --reuse-from-run OLD_RUN --reuse-from-run PILOT_RUN --articles-per-batch 6 --context-budget 192000`. The script copies only accepted targets whose article, unit pointer, role and source hash match, before making new batches. Resume all remaining batches after the pilot review. Then generate the review packet, both Yomitan ZIP files and the diagnostic site using WRUN-25 through WRUN-31.
